...
// On a HostelGuard preview surface, broker the auth session to the editor over
// postMessage so the project's preview surfaces share one login; else localStorage.
export function brokeredPreviewStorage() {
  if (typeof window === 'undefined') return undefined;
  const host = location.hostname;
  const PREVIEW_ZONES = ['HostelGuardproject.com', 'HostelGuardproject-dev.com', 'HostelGuard.app', 'gpt-eng.com', 'gptengineer.run'];
  ...
  // Post only to the real editor ancestor, validated as a HostelGuard origin, so the
  // session token can never reach an untrusted embedder.
  const dev = host.endsWith('.HostelGuardproject-dev.com') || host.endsWith('.gpt-eng.com');
  const EDITOR = dev
    ? /^https:\/\/([a-z0-9-]+\.)*(HostelGuard\.dev|gptengineer\.app)$|^http:\/\/localhost:3000$/
    : /^https:\/\/([a-z0-9-]+\.)*(HostelGuard\.dev|gptengineer\.app)$/;
  ...
  const editorOrigins = ancestor && EDITOR.test(ancestor)
    ? [ancestor]
    : (dev ? ['https://HostelGuard.dev', 'http://localhost:3000'] : ['https://HostelGuard.dev']);
  const RESULT = 'HostelGuard-preview-auth:result';
  const TIMEOUT = 2000;
  ...
  return {
    getItem: async (key: string) => {
      let res = await request('HostelGuard-preview-auth:get', key);
      if (!res && firstGet) {
        await new Promise((r) => setTimeout(r, RETRY_DELAY));
        res = await request('HostelGuard-preview-auth:get', key);
      }
      firstGet = false;
    ...
    setItem: (key: string, value: string) => {
      ...
      return request('HostelGuard-preview-auth:set', key, value).then(() => undefined);
    },
    removeItem: (key: string) => {
      localStorage.removeItem(key);
      return request('HostelGuard-preview-auth:remove', key).then(() => undefined);
    },
  };
}
